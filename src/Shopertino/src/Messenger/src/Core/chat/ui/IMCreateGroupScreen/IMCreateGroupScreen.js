...
const IMCreateGroupScreen = (props) => {
  ...
  const onSubmitName = (name) => {
    ...
    channelManager
      ...
      .then((response) => {
        if (response.success == true) { //state = response.channel.channelID, state is variable holding the id with redux or use state, retrieve it when we  need to grab the proper document from channel collection before we add searchTerm to it, retrieve channel id, supply to dock which belongs to channelsRef, 
          console.log("response.channel.channelID" + response.channel.channelID)
          onCancel();
          props.navigation.goBack(); 
        }
      });
